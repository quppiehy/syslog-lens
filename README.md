...
- **Routing every request through it:** `vercel.json` has a single catch-all rewrite, `"/:path*"` → `"/api?__path=:path*"`, so every path (`/`, `/login`, `/auth.js`, `/api/auth/login`, everything) reaches `api/index.js`. **`rewrites[].source` is path-to-regexp syntax, not a raw anchored regex:** an earlier version of this rewrite used `"^/(.*)$"`, which isn't valid path-to-regexp syntax and silently matched nothing in production — `GET /` and `GET /login` came back as Vercel's own `404 NOT_FOUND` while `GET /api` worked (it's served directly by the function, not via the rewrite). The [vercel.json rewrites reference](https://vercel.com/docs/project-configuration/vercel-json#rewrites) shows named parameters instead — `/resize/:width/:height` → `/api/sharp` (captured segments become query parameters on the destination, not extra path segments) and `/proxy/:match*` → `https://example.com/:match*` (a named zero-or-more-segments parameter substituted by name into the destination). So the rewrite above hands `api/index.js` a request whose `req.url` is `/api` plus a `__path` query parameter holding the path the browser actually requested (an empty string for `/`, since `:path*` can match zero segments). **Preserving the original path across the rewrite:** `api/index.js` reads `req.query.__path` (one of the Node.js helper properties [Vercel populates on the request object](https://vercel.com/docs/functions/runtimes/node-js#node.js-helpers)), reassembles the real `req.url` (path plus any other query parameters), and only then hands the request to the Express app — see the comments in `api/index.js` and the tests in `test/vercel-entry.test.js` for the exact reassembly logic and its coverage (root path, nested paths, preserved query parameters, and an end-to-end HTTP request that resolves `/login` correctly). A request to `/api` itself is unaffected: per the same docs page, rewrites "check the filesystem by default," and `/api` is itself a filesystem function route.
