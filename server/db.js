// Sequelize connection setup. Uses Postgres (via `pg`) when DATABASE_URL is
// set (e.g. the Neon integration on Vercel), otherwise SQLite (via
// `sqlite3`), exactly as before — see server/lib/dbConfig.js for the
// dialect-selection logic and why each driver is required lazily.
const fs = require('fs');
const path = require('path');
const { Sequelize } = require('sequelize');
const { DB_STORAGE, DATABASE_URL } = require('./config');
const { getSequelizeOptions } = require('./lib/dbConfig');

let sequelize;

if (DATABASE_URL) {
  sequelize = new Sequelize(DATABASE_URL, getSequelizeOptions({ databaseUrl: DATABASE_URL }));
} else {
  ...
  sequelize = new Sequelize(getSequelizeOptions({ sqliteStorage: DB_STORAGE }));
